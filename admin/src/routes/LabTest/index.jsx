  ...
  const toggleExpand = (key) => {
    ...
    const columns = [
      ...
      render: (description, record) => {
        ...
        // Replace the existing preparation column (around line 115-135) with this:
{
    title: "Preparation",
    ...
    const fetchLabTests = async () => {
        try {
            ...
            if (response?.responseCode === 200) {
                ...
                // Apply status filter
                if (selectedStatus) {
                    filteredData = filteredData.filter(test => test.status === selectedStatus);
                }

                // Apply category filter
                if (selectedCategory) {
                    filteredData = filteredData.filter(test => test.category?._id === selectedCategory);
                }
    ...
    useEffect(() => {
        fetchLabTests();
    }, [currentPage, pageSize, selectedStatus, selectedCategory, searchQuery]);
